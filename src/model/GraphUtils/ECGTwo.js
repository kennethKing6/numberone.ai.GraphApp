...
export class ECG {
    ...
    addECGValue(ecgData){
        if(this.norm_array.length === 10)this.emptyNormArray();

        this.norm_array.push(ecgData);
    }

    isReady(){
        return this.norm_array.length === 10;
    }
    ...
    emptyNormArray(){
        this.norm_array.length = 0;

    }
    isNormArrayEmpty(){
        return this.norm_array.length == 0;
    }
    getDataCursor() {
    ...
    tick() {
        ...
        if(this.data_buffer.length == 1) this.isLastXY = true;
